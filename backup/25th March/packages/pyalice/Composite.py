...
def parse_composite_message(message: Message.MessageReader, quantities: List):
    ...
    # print('LOGDEBUG  ---------- Length/Count of quantities {}'.format(count))
    indices = [None] * count
    ...
    # print('LOGDEBUG ---------- Priniting entire message.json : {}'.format(message.json))
    offset = 0
    for q in message.json["quantities"]:
        ...
        # print('LOGDEBUG  ----------  d is ',d)
        # print('LOGDEBUG  ---------- PRINTING ENTIRE QUANTITIES : ', quantities)
        for i in range(count):
            p = quantities[i]
            # print('LOGDEBUG ---------- Printing p and q : {} and {} '.format(p,q))
