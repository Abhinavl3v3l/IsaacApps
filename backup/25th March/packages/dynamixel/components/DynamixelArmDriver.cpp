...
namespace isaac
{
  namespace dynamixel
  {
    ...
    void DynamixelArmDriver::parseCommand()
    {
      ...
      LOG_DEBUG("Command 0 in radians is  %f  and goal position is %f ", command(0), static_cast<float>(RadToDeg(command(0))) );
      ...
      goal_position = static_cast<float>(RadToDeg(command(5))) * conv;
      command_to_arm_.push_back(goal_position);

      goal_position = static_cast<float>(RadToDeg(command(6))) * conv;
      command_to_arm_.push_back(goal_position);
    ...
    void DynamixelArmDriver::publishState()
    {
      ...
      // const unsigned int speed = get_speed();
      ...
      for (size_t i = 0; i < servo_ids_.size(); i++)
      {
        ...
        if (servo_id == 9 || servo_id == 11)
        {
          ...
          // current_position_back = static_cast<float>(DegToRad((current_position_back)));
          const int current_speed_back =
