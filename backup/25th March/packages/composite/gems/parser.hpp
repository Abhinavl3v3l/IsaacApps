...
class Parser {
 public:
  ...
  // Parses the composite as a composite view. The number of elements of the requested state must
  // match the number of elements in the requested schema.
  template <template<template<int> class> class State>
  bool parse(::CompositeProto::Reader reader, const std::vector<SharedBuffer>& buffers,
             TypedCompositeView<const double, State>& composite);
...
bool Parser::parse(::CompositeProto::Reader reader, const std::vector<SharedBuffer>& buffers,
                   TypedCompositeView<const double, State>& composite) {
  // Make sure composite is null in case we return with an error
  composite = TypedCompositeView<const double, State>();
  ...
  // Check that tensor dimensions are compatible with the schema
  const int src_state_count = values.dimensions()[1];
  if (src_state_count != received_schema_->getElementCount()) {
    return false;
  }
  const int dst_state_count = requested_schema_.getElementCount();
  constexpr int kExpectedStateCount = TypedCompositeStateDimension<State>;
  if (kExpectedStateCount != dst_state_count) {
    return false;
  }

  // Get indices of quantities
  std::array<int, kExpectedStateCount> offsets;
  fragment_index_->getSourceIndices(offsets.data(), kExpectedStateCount);

  // Construct the result composite based on the tensor
  composite = TypedCompositeView<const double, State>(
      values.element_wise_begin(), values.dimensions()[0], src_state_count, offsets);
  return true;
}
