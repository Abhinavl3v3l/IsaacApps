...
class DynamixelRarmDriver : public alice::Codelet {
 public:
  void start() override;
  void tick() override;
  void stop() override;
  ...
  //Constant speed for all servos of same type
  ...
  // Go Home Initial Position 
  ISAAC_PARAM(bool, gohome, false);
 ...
 private:
  ...
  // Dynamixel
  std::unique_ptr<Dynamixel> dynamixel_64;
  std::unique_ptr<Dynamixel> dynamixel_106;
  std::unique_ptr<Dynamixel> dynamixel_430;
  // Ids of motors we are working with
  std::vector<uint8_t> servo_ids_;

  std::vector<float> command_to_arm_;
...
ISAAC_ALICE_REGISTER_CODELET(isaac::dynamixel::DynamixelRarmDriver);
