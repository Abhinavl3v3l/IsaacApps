#include "DynamixelRarmDriver.hpp"
#include <string>
#include <utility>
#include <vector>
...
namespace isaac
{
  namespace dynamixel
  {
     ...
     * 
     **/
    void DynamixelRarmDriver::start()
    {
    ...
    void DynamixelRarmDriver::stop()
    {
      disableDynamixels();
    }

    bool DynamixelRarmDriver::initializeServoIds()
    {
    ...
    bool DynamixelRarmDriver::goHome()
    {
      bool success = true;
      for (const uint8_t servo : servo_ids_)
      {
        ...
        if (servo == 12 || servo == 16 || servo == 18 || servo == 20)
        {
          if (servo == 12)
          {
    ...
    bool DynamixelRarmDriver::enableDynamixels(DynamixelMode mode)
    {
    ...
    bool DynamixelRarmDriver::validateKinematicTree(const kinematic_tree::KinematicTree &model)
    {
    ...
    void DynamixelRarmDriver::initStateSchema()
    {
    ...
    void DynamixelRarmDriver::initCommandParser()
    {
    ...
    void DynamixelRarmDriver::parseCommand()
    {
      ...
      {
        if (servo_ids_[i] == 12 || servo_ids_[i] == 16 || servo_ids_[i] == 18 || servo_ids_[i] == 20)
        {
    ...
    //Publish Command
    void DynamixelRarmDriver::publishState()
    {
      ...
      for (size_t i = 0; i < servo_ids_.size(); i++)
      {
        ...
        if (servo_id == 12 || servo_id == 16 || servo_id == 18 || servo_id == 20)
        {
    ...
    void DynamixelRarmDriver::disableDynamixels()
    {
      for (const uint8_t servo : servo_ids_)
      {
